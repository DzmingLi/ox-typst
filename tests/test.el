...
(defun org-typst-test-run ()
  ...
  (load (file-name-concat (vc-git-root default-directory) "tests/custom-link.el"))
  (let ((stats (ert-run-tests-batch "^org-typst-\\(?:block\\|link\\)-")))
    (unless (zerop (ert-stats-completed-unexpected stats))
      (error "Special block or custom link tests failed")))
